...
// Booking destination (internal). Swap to external scheduler URL when ready.
const DESTINATION_URL = "/schedule";

const CHECKLIST_ITEMS = ["Please have your ID ready", "Have insurance card ready"];
...
function SaveSpotModalContents({ onClose }: Pick<SaveSpotModalProps, "onClose">) {
          ...
          <ul className="space-y-3 mb-8" aria-label="Before you go">
            {CHECKLIST_ITEMS.map((item) => (
              <li key={item} className="flex items-center gap-3 text-slate-700">
                <CheckCircle2 className="h-5 w-5 shrink-0 text-primary-blue" aria-hidden />
...
export function SaveSpotModal({ isOpen, onClose }: SaveSpotModalProps) {
  return <AnimatePresence>{isOpen ? <SaveSpotModalContents onClose={onClose} /> : null}</AnimatePresence>;
}
